# Tsinghua Literature Downloader — v3.3 (strategy-hardcoded)

通过已登录的清华图书馆/WebVPN Chrome 会话下载学术 PDF。

**v3.3 核心改进：写死单策略 + fail-fast 哨兵。** 每库写死一种下载策略（fetch / click-download / navigate-download / click-human-assisted），无 click↔fetch fallback。列表按库分类后逐库下载，每库第一篇当哨兵——走不通即整库中断交用户。继承 v3 token 受控探针库（`scripts/probe.mjs` → 紧凑 ~200B 判定结果）+ v3.2 网络安全硬规则。
...
├── start.js                  ← 一键启动入口
├── SKILL.md                  ← Claude 读取的技能说明（v3.3 写死策略 + 哨兵 + Token Discipline + 网络安全硬规则）
├── README.md                 ← 本文件
├── journal_database_mapping.md  ← 期刊代码 → 首选数据库 对照表（Stage A 分类输入，v3.3 新增）
├── strategies.tsv            ← 策略清单：库/子类型/策略/last_tested/updated_at（v3.3 新增）
├── canaries.tsv              ← 离线回归测试集（每库一个已知好案例，验证策略未退化）
├── lessons.md                ← 各出版社 playbook（自包含，inline）
├── scripts/                  ← 下载 / 探针工具
│   ├── cdp-client.mjs        ← 共享 CDP HTTP 客户端
│   ├── probe.mjs             ← 探针运行器，JS 服务端驻留
│   ├── browser_pdf_downloader.mjs
│   ├── cdp_open_url.mjs
│   └── extract_pdf_text.py
├── probes/                   ← 紧凑探针库，每探针返回固定小 schema
│   ├── index.mjs             ← 名称 → 探针 注册表
│   ├── classify-page.mjs     ← 组合探针：一次返回 stage/access/cloudflare/captcha/pdfUrl
│   └── <publisher>.mjs       ← 各出版社专属探针
├── actions/                  ← 各出版社"如何取字节"动作库（写死单策略，无 fallback）
│   ├── index.mjs             ← 名称 → 动作 注册表
