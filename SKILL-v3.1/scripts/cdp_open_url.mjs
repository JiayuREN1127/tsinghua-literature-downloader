#!/usr/bin/env node
// Open a URL in the already-authorized Chrome CDP proxy.
// This helper URL-encodes nested URLs correctly, which matters for 水木学术搜索
// URLs containing #!. It does not read cookies, passwords, local storage, or
// browser profiles.
//
// v3: imports shared helpers from ./cdp-client.mjs (no more duplicated code).

import { proxyGet, proxyPost, sleep, normalizeProxy } from "./cdp-client.mjs";
...
This helper URL-encodes nested URLs correctly, which matters for 水木学术搜索 URLs containing #!.
...
async function main() {
  ...
  const created = await proxyPost(proxy, "/new", args.url, 60000);
  const target = created.targetId;
  const info = args.wait
    ? await waitForComplete(proxy, target)
    : await proxyGet(proxy, "/info", { target }, 10000);
